...
/****** Object:  StoredProcedure [dbo].[GET_EMPLOYEE_EMAIL_ID]    Script Date: 03/30/2021 9:00:45 PM ******/
IF EXISTS (
		SELECT *
		FROM sys.objects
		WHERE object_id = OBJECT_ID(N'[dbo].[GET_EMPLOYEE_EMAIL_ID]')
			AND type IN (
				N'P'
				,N'PC'
				)
		)
	DROP PROCEDURE [dbo].GET_EMPLOYEE_EMAIL_ID
GO
...
/****** Object:  StoredProcedure [dbo].[ADD_EMPLOYEE_PAYROLL_DETAILS]    Script Date: 2/9/2021 5:45:12 PM ******/
SET ANSI_NULLS ON
GO
...
BEGIN  
 ...
 BEGIN  
  UPDATE Employee  
  SET [IS_PF_RESTRICTION_APPLICABLE] = @IS_PF_RESTRICTION_APPLICABLE    WHERE id = @EMP_ID  
 END  
END  
GO
...
/****** Object:  StoredProcedure [dbo].[INSERT_UPDATE_IT_DECLARATION_HEADER]    Script Date: 2/9/2021 5:36:11 PM ******/
SET ANSI_NULLS ON
GO
...
BEGIN
	...
	BEGIN
		...
		SET IT_Declaration_Enabled = @IT_Declaration_Enabled
...
CREATE PROCEDURE [dbo].[MTS_ADDUSER] @USERNAME VARCHAR(150)
	,@PASSWORD VARCHAR(50)
...
CREATE PROCEDURE [dbo].[MTS_CHECK_EXISTS_USERNAME] (@USERNAME VARCHAR(150))
AS
BEGIN
	IF EXISTS (
			SELECT 1
...
/****** Object:  StoredProcedure [dbo].[MTS_GET_IT_DECLARATION_EMPLOYEES]    Script Date: 2/9/2021 5:39:54 PM ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [dbo].[MTS_GET_IT_DECLARATION_EMPLOYEES] (
	@START_YEAR BIGINT
	,@END_YEAR BIGINT
	)
AS
BEGIN
	...
	SELECT E.id
		...
		,CASE 
			...
			WHEN Submitted_on IS NOT NULL
				AND Proof_Submitted_on IS NULL
				THEN CONVERT(VARCHAR(10), Submitted_on, 101)
			WHEN Submitted_on IS NOT NULL
				AND Proof_Submitted_on IS NOT NULL
				AND Approved_On IS NULL
				THEN CONVERT(VARCHAR(10), Proof_Submitted_on, 101)
			ELSE NULL
...
CREATE PROCEDURE [dbo].[MTS_GETUSER] (@LOGIN VARCHAR(150))
AS
BEGIN
	SET NOCOUNT ON;
...
/****** Object:  StoredProcedure [dbo].[MTS_PAYROLL_GET_FROM_MONTH]    Script Date: 2/9/2021 5:41:33 PM ******/
SET ANSI_NULLS ON
GO
...
BEGIN  
 ...
 BEGIN  
  SELECT CONVERT(VARCHAR(10), DATEADD(MONTH, - 1, CAST(@MONTH + '-25-' + @YEAR AS DATE)), 101)  
 END  
 ELSE  
 BEGIN  
  IF EXISTS (  
  ...
  BEGIN  
   ...
   SELECT CONVERT(VARCHAR(10), @ATTENDANCE_DATE_FROM, 101)  
  END  
  ELSE  
  BEGIN  
   SELECT CONVERT(VARCHAR(10), DATEADD(MONTH, - 1, CAST(@MONTH + '-25-' + @YEAR AS DATE)), 101)  
...
/****** Object:  StoredProcedure [dbo].[MTS_PAYROLL_GET_TO_MONTH]    Script Date: 2/9/2021 5:43:12 PM ******/
SET ANSI_NULLS ON
GO
...
BEGIN  
 ...
 BEGIN  
  SELECT CONVERT(VARCHAR(10), @MONTH + '-25-' + @YEAR, 101)  
 END  
 ELSE  
 BEGIN  
  IF EXISTS (  
  ...
  BEGIN  
   ...
   SELECT CONVERT(VARCHAR(10), @ATTENDANCE_DATE_TO, 101)  
  END  
  ELSE  
  BEGIN  
   SELECT CONVERT(VARCHAR(10), @MONTH + '-25-' + @YEAR, 101)  
...
CREATE PROCEDURE [dbo].[MTS_USERPASSWORDEMAIL] @To AS VARCHAR(150)
...
CREATE PROCEDURE [dbo].[GET_EMPLOYEE_EMAIL_ID]
AS
BEGIN
	SELECT USERNAME AS Employee_Email_Id 
	FROM MTS_LOGIN WITH (NOLOCK)
	WHERE ACTIVE = 1
END
GO