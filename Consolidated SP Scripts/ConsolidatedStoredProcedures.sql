...
CREATE PROCEDURE [dbo].[MTS_GET_IT_DECLARATION_EMPLOYEES] (@YEAR BIGINT)
AS
BEGIN
	DECLARE @START_DATE DATE
		,@END_DATE DATE

	SET @START_DATE = CAST(CAST(@YEAR AS VARCHAR) + '-04-01' AS DATE)
	SET @END_DATE = EOMONTH(CAST(CAST(@YEAR + 1 AS VARCHAR) + '-03-01' AS DATE));
